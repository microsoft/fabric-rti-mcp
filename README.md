...
| `RTI_INSTRUCTIONS_URI` | Common | Enables instruction tools and points to a folder of `.md` files (file URI) | None | `file:///C:/rti/instructions/` |
| `RTI_INSTRUCTIONS_DESCRIPTION` | Common | JSON controlling list description generation (`head` or `metadata`) | None | `{"method":"head","params":{"n":200}}` |

### Instruction Loading (Recipes)

The instruction loading mechanism is a convenience feature for scenarios where you have many complex workflows ("recipes") and you want an agent to discover and combine them with the rest of the MCP tools.

Instead of embedding all workflow guidance in a single prompt, you can store multiple markdown files and let the agent:
- call `common_instructions_list()` to browse what is available (with short descriptions)
- call `common_instructions_load(name)` to load a specific recipe and apply it

At the moment, this feature supports the local filesystem only (via `RTI_INSTRUCTIONS_URI` as a `file://` URI). It is designed to be expanded to other storage solutions later (e.g., git-based sources, Azure Storage, or even Kusto-backed catalogs).

**Example recipe file (markdown)**

Create a file like `weather-forecast-task.md` under your instructions folder (the folder pointed to by `RTI_INSTRUCTIONS_URI`), see example file under `examples/instructions/weather-forecast-task.md` in the repo.
