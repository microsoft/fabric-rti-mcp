...
def generate_teams_binding(alert_recipient: str, headline: str, message: str) -> str:
    ...
    {{
        ...
        "arguments":[
            ...
            {{"name":"recipients","type":"array","values":[{{"type":"string","value":"{alert_recipient}"}}]}},
...
def generate_email_binding(alert_recipient: str, headline: str, message: str) -> str:
    ...
    return f"""
        {{
            ...
            "arguments":[
                {{"name":"messageLocale","type":"string","value":""}},
                {{"name":"sentTo","type":"array","values":[{{"type":"string","value":"{alert_recipient}"}}]}},
                {{"name":"copyTo","type":"array","values":[]}},
                {{"name":"bCCTo","type":"array","values":[]}},
                {{"name":"subject","type":"array","values":[{{"type":"string","value":"{headline}"}}]}},
                {{"name":"headline","type":"array","values":[{{"type":"string","value":"{headline}"}}]}},
                {{"name":"optionalMessage","type":"array","values":[{{"type":"string","value":"{message}"}}]}},
                {{"name":"additionalInformation","type":"array","values":[]}}]
        }}"""
...
def create_container_entity(trigger_name: str) -> tuple[dict[str, Any], str]:
    ...
    container: dict[str, Any] = {
        ...
        "payload": {"name": f"{trigger_name}", "type": "unconstrained"},
        "type": "container-v1",
    }
...
def create_kql_source_entity(
    ...
    source: dict[str, Any] = {
        ...
        "payload": {
            ...
            "runSettings": {"executionIntervalInSeconds": polling_frequency_seconds},
            "query": {"queryString": kql_query},
            "eventhouseItem": {"databaseName": database, "clusterHostName": cluster_hostname},
            "queryParameters": [],
        ...
        "payload": {
            ...
            "definition": {
                ...
                "instance": f"""{{
                    ...
                    "steps":[
                        {{
                            "name":"SourceEventStep","id":"{str(uuid.uuid4())}",
                            "rows":[
                                {{
    ...
    event_rule_entity: dict[str, Any] = {
        ...
        "payload": {
            ...
            "definition": {
                ...
                "settings": {"shouldRun": True, "shouldApplyRuleOnUpdate": False},  # Enable the rule
            },
        },
        "type": "timeSeriesView-v1",
    }
